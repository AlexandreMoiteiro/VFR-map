...
# --- CSS: nuke any opacity/filters and set dark bg under map for contrast ---
st.markdown("""
<style>
html, body, .stApp { opacity: 1 !important; filter: none !important; background: #0b0b0b !important; }
...
# ---------- Header ----------
st.markdown("<h1 style='margin-bottom:0;color:#fff'>Portugal VFR — Localidades + AD/HEL/ULM</h1>", unsafe_allow_html=True)
st.markdown("<div style='opacity:0.95;margin-top:2px;margin-bottom:14px;color:#ddd'>App by <b>Alexandre Moiteiro</b></div>", unsafe_allow_html=True)
...
# ---------- Satellite basemap (Esri Imagery + labels overlay) ----------
...
if show_loc and not loc_f.empty:
    ...
    for _, r in loc_f.iterrows():
        ...
        label_html = (
            ...
            'border:1px solid rgba(255,255,255,0.25);">{}</div>'.format(code)
        )
        folium.Marker(
