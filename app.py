...
st.markdown("<div style='opacity:0.9;margin-top:2px;margin-bottom:14px'>App by <b>Alexandre Moiteiro</b></div>", unsafe_allow_html=True)
...
m = folium.Map(location=[mean_lat, mean_lon], zoom_start=6, tiles=sat_tiles, attr=sat_attr, control_scale=True)

# ---------- Layers ----------
# Localidades — mostrar apenas o código (5 letras) no mapa; nome/sector no hover
if show_loc and not loc_f.empty:
    ...
    for _, r in loc_f.iterrows():
        ...
        label_html = (
            ...
            'border:1px solid rgba(255,255,255,0.25);backdrop-filter:blur(2px);">{}</div>'.format(code)
        )
        folium.Marker(
